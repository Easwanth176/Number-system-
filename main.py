...
def GetSave():
    global l32
    clear_frame()
    l33=tk.Label(root,text="STORED ENTRIES",font=("Arial",30),bg=x)
    l33.place(x=270,y=45) 
    l31=tk.Label(root,text="Enter number        : ",font=("Arial",15),bg=x)
    l31.place(x=220,y=180)
    l32=tk.Entry(root,width=20,font=("Arial",15))
    l32.place(x=420,y=180) 
    button=Button(root,text="home",width=10,height=2,command=home).place(x=280,y=270)
    button=Button(root,text="get details",width=10,height=2,command=printValues).place(x=520,y=270)
...
def printValues():
    ...
    mydb = mysql.connector.connect(user='root',host='localhost',password='Password',database='cherry')
    mycursor=mydb.cursor()
    mycursor.execute("select * from conversion where id='%s' "%(number))
    values=mycursor.fetchall()   
    if number.isdigit()==False or len(number)!=4:
        msg='Enter a valid number'
        messagebox.showerror('Error',msg)
    if values==[]:
        msg='No such number exists'
        messagebox.showerror('Error',msg)
    if msg=='':
       
        l34=tk.Label(root,text="Your Id : ",font=("Arial",15),bg=x)
        l34.place(x=220,y=350)
        l35=tk.Label(root,text=values[0][0],font=("Arial",15),bg=x)
        l35.place(x=420,y=350)
        l36=tk.Label(root,text="Number : ",font=("Arial",15),bg=x)
        l36.place(x=220,y=400)
        l37=tk.Label(root,text=values[0][1],font=("Arial",15),bg=x)
        l37.place(x=420,y=400)
        l38=tk.Label(root,text="Selected type : ",font=("Arial",15),bg=x)
        l38.place(x=220,y=450)
        l39=tk.Label(root,text=values[0][2],font=("Arial",15),bg=x)
        l39.place(x=420,y=450)
        l40=tk.Label(root,text=" Binary : ",font=("Arial",15),bg=x)
        l40.place(x=220,y=500)
        l41=tk.Label(root,text=values[0][3],font=("Arial",15),bg=x)
        l41.place(x=420,y=500)
        l42=tk.Label(root,text="Octal : ",font=("Arial",15),bg=x)
        l42.place(x=220,y=550)
        l43=tk.Label(root,text=values[0][4],font=("Arial",15),bg=x)
        l43.place(x=420,y=550)
        l44=tk.Label(root,text="Decimal : ",font=("Arial",15),bg=x)
        l44.place(x=220,y=600)
        l45=tk.Label(root,text=values[0][5],font=("Arial",15),bg=x)
        l45.place(x=420,y=600)
        l46=tk.Label(root,text="Hexadecimal : ",font=("Arial",15),bg=x)
        l46.place(x=220,y=650)
        l47=tk.Label(root,text=values[0][6],font=("Arial",15),bg=x)
        l47.place(x=420,y=650)


def save():
    try:
        import mysql.connector
        mydb = mysql.connector.connect(user='root',host='localhost',password='Easwanth@1647765',database='cherry')
        mycursor=mydb.cursor()
        mycursor.execute("CREATE TABLE conversion(id int primary key,number VARCHAR(55),type VARCHAR(55),binary_ VARCHAR(55),octal VARCHAR(55),decimal_ VARCHAR(55),hexadecimal VARCHAR(55))")
        mydb.commit()
    except:
        pass
    ...
    if st=="decimal":
        mycursor.execute("INSERT INTO conversion(id,number,type,binary_,octal,decimal_,hexadecimal) VALUES(%s,%s,%s,%s,%s,%s,%s)",(id,number,type,ans,ans1,number,ans2))
        mydb.commit()
        ...
        mycursor.execute("INSERT INTO conversion(id,number,type,binary_,octal,decimal_,hexadecimal) VALUES(%s,%s,%s,%s,%s,%s,%s)",(id,number,type,number,ans3,ans4,ans5))
        mydb.commit()
        msg="sucessfully saved \n your id :"+str(id)
        messagebox.showinfo("message", msg)
        home()
        ...
        mycursor.execute("INSERT INTO conversion(id,number,type,binary_,octal,decimal_,hexadecimal) VALUES(%s,%s,%s,%s,%s,%s,%s)",(id,number,type,ans6,number,ans7,ans8))
        mydb.commit()
        msg="sucessfully saved \n your id :"+str(id)
        messagebox.showinfo("message", msg)
        home()
        ...
        mycursor.execute("INSERT INTO conversion(id,number,type,binary_,octal,decimal_,hexadecimal) VALUES(%s,%s,%s,%s,%s,%s,%s)",(id,number,type,ans9,ans10,ans11,number))
        mydb.commit()
        msg="sucessfully saved \n your id :"+str(id)
        messagebox.showinfo("message", msg)
        home()
...
def home():
  ...
  label=tk.Label(root, bg=x, text="HOME PAGE",font = ('Latin Modern Roman Italic bold underline',30))
  ...
  button=Button(root,text="Result",width=10,height=2,bg="white",command=lambda:[validation()]).place(x=300,y=500)
  button1=Button(root,text="View Saved",width=10,height=2,bg="white",command=GetSave).place(x=500,y=500)
...
def result():
    ...
    button=Button(root,text="home",width=10,height=2,command=home).place(x=210,y=480)
    button=Button(root,text="Explanation",width=10,height=2,command=explain).place(x=390,y=480)
    button=Button(root,text="save",width=10,height=2,command=save).place(x=550,y=480)
...
def explain():
    ...
    button=Button(root,text="home",width=10,height=2,command=home).place(x=320,y=600)
    button=Button(root,text="Save",width=10,height=2,command=save).place(x=520,y=600)
